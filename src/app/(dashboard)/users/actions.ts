...
export async function migrateEmailsAndRoles() {
  try {
    const adminAuthClient = getAdminClient()
    const supabaseServer = await createServerClient()
    ...
    // Fetch all bidang
    const { data: bidangList } = await supabaseServer.from('bidang').select('*')
    ...
    for (const u of targetUsers) {
      ...
      const { error: updateError } = await adminAuthClient.auth.admin.updateUserById(u.id, { email: newEmail })
      ...
      if (bidangMatch) {
        const { error: metaError } = await adminAuthClient.from('users_metadata').upsert({
          id: u.id,
          username: namePrefix,
        ...
        logs.push({ oldEmail, newEmail, status: 'Success', bidangAssigned: bidangMatch.name, metaError: metaError ? metaError.message : null })
      } else {
        // Just insert without bidang if not matched
        const { error: metaError } = await adminAuthClient.from('users_metadata').upsert({
          id: u.id,
          username: namePrefix,
          full_name: `Admin ${namePrefix}`,
          role: 'Admin Bidang',
        })
        logs.push({ oldEmail, newEmail, status: 'Email updated but no Bidang matched', namePrefix, metaError: metaError?.message })
      }
    }
