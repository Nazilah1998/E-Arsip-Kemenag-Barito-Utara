  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="h-10 w-full sm:w-[240px] rounded-xl border-0 bg-slate-50 pl-10 pr-10 text-sm font-medium text-slate-900 ring-1 ring-slate-200 transition-all focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none"
            />
            <button
              onClick={() => setIsFilterOpen(!isFilterOpen)}
              className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-colors ${isFilterOpen || filterType !== 'all' || filterDate !== 'all' ? 'bg-emerald-100 text-emerald-600' : 'text-slate-400 hover:bg-slate-200'}`}
            ...
            {isFilterOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 rounded-2xl bg-white p-4 shadow-xl ring-1 ring-slate-100 z-50 animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-bold tracking-wider text-slate-500 uppercase mb-2 block">Tipe Dokumen</label>
                    <select 
                      value={filterType}
                      onChange={(e) => setFilterType(e.target.value)}
                      className="w-full rounded-xl bg-slate-50 border-0 ring-1 ring-slate-200 text-sm font-medium text-slate-700 py-2 px-3 focus:ring-2 focus:ring-emerald-500"
                    >
                      <option value="all">Semua Tipe</option>
                      <option value="folder">Hanya Folder</option>
                      <option value="image">Gambar (JPG, PNG)</option>
                      <option value="pdf">PDF Document</option>
                      <option value="document">Office (Word, Excel, PPT)</option>
                    </select>
                  </div>
                  <div>
          ...
          <div className="flex items-center gap-1 rounded-xl bg-slate-50 p-1 ring-1 ring-slate-200">
            <button
              ...
              className={`rounded-lg p-2 transition-all ${viewMode === "list" ? "bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200" : "text-slate-400 hover:text-slate-600"}`}
              title="List View"
            >
            ...
            <button
              ...
              className={`rounded-lg p-2 transition-all ${viewMode === "grid" ? "bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200" : "text-slate-400 hover:text-slate-600"}`}
              title="Grid View"
            >
          ...
          <div className="relative">
            <button 
              ...
              className="flex h-10 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-sm font-bold text-white shadow-md shadow-emerald-600/20 transition-all hover:bg-emerald-700 hover:shadow-lg hover:shadow-emerald-600/30 ring-1 ring-emerald-500"
            >
            ...
            {isNewMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl bg-white p-2 shadow-xl ring-1 ring-slate-100 z-50 animate-in fade-in zoom-in-95">
                  <button
                    onClick={() => { setIsCreateOpen(true); setIsNewMenuOpen(false); }}
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors"
                  >
      ...
      <div className="flex items-start gap-4 h-full relative">
        <div className={`transition-all duration-300 flex-1 ${selectedInfoItem ? 'hidden xl:block' : 'block'}`}>
          <FileTable 
            data={initialItems} 
